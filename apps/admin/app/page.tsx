...
import { fmt, fmtBytes, timeAgo, statusBadgeClass, planBadgeClass } from '@/lib/utils'
import type { Tenant, AuditLogEntry } from '@/lib/types'
import {
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_320px]">
        ...
        <div className="admin-card overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: '1px solid var(--border)' }}>
            ...
            <Link href="/tenants" className="flex items-center gap-1 text-xs font-medium" style={{ color: 'var(--accent)' }}>
              View all <ArrowRight className="h-3.5 w-3.5" />
            </Link>
          </div>
          {recentTenants.length === 0 ? (
            <div className="py-12 text-center text-sm" style={{ color: 'var(--text-muted)' }}>
              No companies yet. <Link href="/tenants/new" style={{ color: 'var(--accent)' }}>Onboard the first →</Link>
            </div>
          ) : (
