...
import { CheckCircle2, WifiOff, Wifi, ChevronDown } from 'lucide-react'
...
export function AppSwitcher() {
  ...
  const allConnected = (a: typeof app) =>
          ...
          <div className="absolute left-0 top-full z-50 mt-1.5 w-72 rounded-xl border border-border/80 bg-card shadow-lg shadow-black/20">
            <div className="px-3 pt-3 pb-1">
              <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">
                Connected Apps — switch to demo
              </p>
            </div>
            {allApps.map((a) => {
              const active = a.id === app.id
              const connected = allConnected(a)
              return (
                <button
                  key={a.id}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      {active && <CheckCircle2 className="h-3 w-3 text-intel" />}
                    </div>
                    <p className="text-[10px] text-muted-foreground">
                      {a.featureAreas.map((fa) => fa.label).join(' · ')}
                    </p>
                  </div>
            ...
            <div className="border-t border-border/50 px-3 py-2.5">
              <p className="text-[10px] text-muted-foreground/70">
                Any app can connect to the PAAQ Listening Tool via the Settings tab. The dashboard shell never changes — only the content inside each tab adapts to the registered app.
              </p>
            </div>
          </div>
        </>
